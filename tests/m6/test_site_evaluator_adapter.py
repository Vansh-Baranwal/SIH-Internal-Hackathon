...
def test_blender_click_coordinate_mapping():
    """
    Test that a click at Blender (bx, by, bz) maps correctly to (x, y, z) 
    ...
    bx, by, bz = 123.0, 456.0, 10.0
    
    # Evaluate click
    result = adapter.evaluate_blender_click(bx, by, bz, lander, site_id="CLICK_01")
...
def test_blender_click_mock_feasibility():
    ...
    # Inside 500m
    result_safe = adapter.evaluate_blender_click(0.0, 400.0, 0.0, lander)
    assert result_safe.feasible is True
    assert result_safe.score == 0.9
    
    # Outside 500m
    result_unsafe = adapter.evaluate_blender_click(400.0, 400.0, 0.0, lander)
